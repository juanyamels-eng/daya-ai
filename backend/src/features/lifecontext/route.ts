...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e instanceof Error ? e.message : 'No se pudo cargar el contexto.' })
  }
})
...
router.post('/location', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e instanceof Error ? e.message : 'No se pudo guardar la ubicación.' })
  }
})
...
router.post('/fact', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e instanceof Error ? e.message : 'No se pudo guardar el dato.' })
  }
})
...
router.delete('/fact/:key', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e instanceof Error ? e.message : 'No se pudo borrar el dato.' })
  }
})
