...
router.post('/audit', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e?.message || 'La auditoría falló.' })
  }
})
...
router.get('/skills', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e?.message || 'No se pudieron cargar las skills.' })
  }
})
...
router.patch('/skills/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e?.message || 'No se pudo actualizar la skill.' })
  }
})
...
router.delete('/skills/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: e?.message || 'No se pudo borrar la skill.' })
  }
})
