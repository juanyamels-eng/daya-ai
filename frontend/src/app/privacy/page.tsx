...
export default async function PrivacyPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', fontFamily: 'var(--font-body)' }}>
      <nav style={{ borderBottom: '1px solid var(--border-default)', padding: '16px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-surface)' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '56px 24px 80px' }}>
        <div style={{ marginBottom: 40 }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>{t('tag')}</p>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2.4rem', fontWeight: 400, color: 'var(--text-primary)', letterSpacing: '-0.02em', marginBottom: 12 }}>{t('title')}</h1>
          <p style={{ color: 'var(--text-tertiary)', fontSize: '0.85rem' }}>{t('date')}</p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
          {sections.map((s, i) => (
            <div key={i} style={{ paddingBottom: 32, borderBottom: i < sections.length - 1 ? '1px solid var(--border-default)' : 'none' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', lineHeight: 1.85, fontSize: '0.875rem' }}>{s.content}</p>
            </div>
          ))}
        </div>
      </div>
